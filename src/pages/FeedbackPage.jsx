  ...
  try {
    ...
    const prompt = `
      ...
      ${userData ?
        ...
        피드백은 '범주형 식단평가'와 '현재 식사의 피드백'과 '개선사항'으로 구분됩니다.
        다음 지시사항을 반드시 지켜주세요.:

        공통 규칙
        1. 답변에 '**'와 같은 굵은 글씨 표시는 사용하지 마세요.
        2. 항목 사이를 반드시 두 번의 줄바꿈 문자(\n\n)로 구분하세요.
        3. 모든 항목은 건강하게 먹자는 의미의 저속노화를 기준으로 서술합니다.

        범주형 식단 평가
        1. 첫 번째 문단은 반드시 사용자의 식단 평가를 '이번 식사는 [나쁩니다./좋아요./괜찮습니다].' 형식으로 출력하세요. 
        2. [나쁩니다./좋아요./괜찮아요.] 은 사용자의 신체정보와 현재의 삭단을 저속노화를 기준으로 평가해 선택합니다.
        
        현재 식사의 피드백
        1. 답변을 '현재 식사의 피드백'과 '개선사항'으로 나누세요. '현재 식단의 피드백'과 '개선사항' 항목은 **'키: 값'** 쌍의 형식으로 작성되어야 합니다.
        2. 현재 식사의 피드백은 현재 사용자의 식사가 어떤지 저속노화의 측면으로 1~2문장으로 명시해주세요.
        3. 현재 어떤 영양분이 부족한지, 아니면 충분한지, 아니면 괜찮은지 기술합니다.
        
        개선사항
        1. 개선사항 내용은 저속노화를 위해 앞으로 어떤 영양분이 부족하고 필요한지, 앞으로 어떤 영양분을 줄여야하는지 1~2문장으로 명시해주세요.
        `;
  ...
  try {
    ...
    const prompt = `
      ...
      ${userData ?
        ...
        단순한 식단 평가
        1. 첫 번째 문단은 반드시 사용자의 식단 평가를 '오늘의 식사는 [나쁩니다./좋아요./괜찮아요.].' 형식으로 출력하세요. [나쁩니다./좋아요./괜찮아요.] 은 사용자의 신체정보와 현재 누적된 영양성분을 배경으로 구체적인 영향학적 기준으로 실행합니다.
