  ...
  try {
    ...
    const prompt = `
      ...
      ${userData ?
        ...
        1. 첫 번째 문단은 반드시 사용자의 식단 평가를 '이번 식사는 [나쁩니다./좋아요./괜찮습니다].' 형식으로 출력합니다. 이 뒤에 줄 바꿈(개행)을 한 번 사용합니다.
        [나쁩니다./좋아요./괜찮아요.] 은 사용자의 신체정보와 현재의 삭단을 저속노화를 기준으로 평가해 선택합니다.
        하루 중 한 끼 식사로 가정 후 평가합니다.
        이 후에 항목 사이의 구분을 위해 두 번의 줄바꿈 문자(\n\n)를 넣어주세요.


        2. 답변을 '현재 식단의 피드백'과 '개선사항'으로 나누세요. '현재 식단의 피드백'과 '개선사항' 항목은 **'키: 값'** 쌍의 형식으로 작성되어야 합니다.
        3. 개선사항 내용은 저속노화를 위해 현재 어떤 영양분이 부족하고 필요한지, 앞으로 어떤 영양분을 줄여야하는지 1~2문장으로 명시해주세요.
        4. 두 항목 사이를 반드시 두 번의 줄바꿈 문자(\n\n)로 구분하세요.
        5. 답변에 '**'와 같은 굵은 글씨 표시는 사용하지 마세요.
        `;
